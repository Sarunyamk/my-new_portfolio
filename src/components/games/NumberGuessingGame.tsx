...
const NumberGuessingGame = () => {
  ...
  const handleGuess = () => {
    ...
    if (userGuess === targetNumber) {
      setMessage(`${t('interactive.numberGame.correct')} ${attempts + 1}`)
      setGameWon(true)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t('interactive.numberGame.description')}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <p>
            {t('interactive.numberGame.attempts')} {attempts}
          </p>
        </div>
      </CardContent>
      <CardFooter>
        {!gameWon ? (
          <CustomButton
            onClick={handleGuess}
            text={t('interactive.numberGame.guess')}
          />
        ) : (
          <CustomButton
            onClick={resetGame}
            text={t('interactive.numberGame.playAgain')}
          />
        )}
      </CardFooter>
    </Card>
