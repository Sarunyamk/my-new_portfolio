...
const MemoryGame: React.FC = () => {
  ...
  const iconColorClass =
    theme === 'dark' ? 'text-primary-foreground' : 'text-zinc-800'
        ...
        {!gameStarted ? (
          <div className="flex justify-center items-center mx-auto">
            <CustomButton
              ...
              text={t('interactive.memory.start')}
            />
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex justify-between mb-2 text-sm font-medium">
              <span>
                {t('interactive.memory.moves')}: {moves}
              </span>
            </div>
              ...
              {cards.map((card) => (
                <motion.div
                  ...
                  className={`aspect-square rounded-md shadow-md flex items-center justify-center transition-all duration-300
                    ...
                    cursor-pointer`}
                  whileHover={{
                    scale: card.isMatched || card.isFlipped ? 1 : 1.05,
                  }}
                  animate={{
